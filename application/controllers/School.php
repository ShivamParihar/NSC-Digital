...
class School extends MY_Controller{
	...
	//function to check user in school or not
    ...
    //function to display add operator page
	public function addOperator(){
		
		$this->load->view('add_operator');
	}
    
    //function to display add accountant page
	public function addAccountant(){
		
		$this->load->view('add_accountant');
	}
    
    //function to display operator and accountant list
	...
	public function operator_register(){
		...
		$this->form_validation->set_rules('fee_salary','Salary','trim|integer');
		...
		if( $this->form_validation->run() == TRUE){
			...
			if( $q->num_rows()>0 ){
				...
				return redirect('School/addOperator');
			}
    		...
    		if($row_affected > 0){
                ...
                if($email !=''){
                    ...
                    $message = '<p style="color: #000;">Congratulations!! You have successfully registered an account on NSC Digital as Operator.</p>';
	...
	public function accountant_register(){
		...
		if( $this->form_validation->run() == TRUE){
    		...
    		if($row_affected > 0){
                ...
                $senderId = "NSCDIG";
                //Your message to send, Add URL encoding here.
                $message = urlencode("NSC Digital\nRegistration Successful (Accountant)\n\nMobile No. : ".$contact_number."\nPassword : ".$password);
                //$message = urlencode("NSC Digital");
                //Prepare you post parameters
                $postData = array(
                    'authkey' => $authKey,
                ...
                if($email !=''){
                    ...
                    $this->email->to($to_email);
                    $this->email->subject('Login Details - NSC Digital');
                    $message = '<p style="color: #000;">Congratulations!! You have successfully registered an account on NSC Digital as Accountant.</p>';
                    $message .='<h4 style="color: #000;">Your login details : </h4>';
                    $message .= 'Contact number : '.$contact_number;
                    $message .= '<br/>Password : '.$password.'<br/><br/>';
                    $message .= '<a href="ihisaab.co.in/DSC/index.php/LoginController" style="background-color: orange;border: none;
    		...
    		return redirect('School/addAccountant');
		}
		else{
			//if validation fails with errors
			$this->load->view('add_accountant');
